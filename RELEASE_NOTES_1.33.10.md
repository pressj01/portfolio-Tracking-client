# Portfolio Tracking Client v1.33.10
...
v1.33.6 was prepared but never published, so everything described for it is included here and is new to anyone coming from v1.33.5. v1.33.8 corrected the realized total on the Option Trades summary, which reported only the current year while every other card on it counted every trade. v1.33.9 adds a backup and restore for your cash-flow plan, so the expenses you typed in by hand can be saved to a file and loaded back. v1.33.10 makes Retirement Readiness and Cash Flow & Sustainability report the same monthly expenses, and stops the plan reporting more than the bills you entered.
