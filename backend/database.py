...
def ensure_tables_exist(conn=None):
    ...
    # Runs after cash_flow_items exists — the backfill below reads it, so on a
    # brand-new database an earlier placement fails the whole initialization.
    plan_cols = [r[1] for r in cur.execute("PRAGMA table_info(cash_flow_plans)").fetchall()]
    if "source_plan_id" not in plan_cols:
