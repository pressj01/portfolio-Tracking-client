...
def fetch_prices(tickers: List[str], start: str, end: str
    ...
    # yfinance treats ``end`` as exclusive while the tester's date input is
...
def simulate_actual_history(history: dict, initial: float) -> Dict[str, pd.Series]:
    """Adapt the shared transaction-aware return index to tester output.

    ``history`` is server-generated by app.py from the same tracker replay used
    by Growth and Total Return.  Scaling its index to ``initial`` makes the real
    portfolio directly comparable with hypothetical A/B portfolios without
    pretending the account's deposits or withdrawals were investment returns.
    """
    dates = list((history or {}).get("dates") or [])
    values = list((history or {}).get("values") or [])
    if len(dates) != len(values) or len(dates) < 2:
        raise ValueError("Actual portfolio history has fewer than two usable observations.")

    frame = pd.DataFrame({"date": pd.to_datetime(dates, errors="coerce"), "index": values})
    frame["index"] = pd.to_numeric(frame["index"], errors="coerce")
    frame = (
        frame.dropna(subset=["date", "index"])
    ...
    tracker = pd.Series(
        frame["index"].astype(float).to_numpy(),
        index=pd.DatetimeIndex(frame["date"]),
        name="value",
    )
    value = tracker / float(tracker.iloc[0]) * float(initial)
    running_max = value.cummax()
    drawdown = (value - running_max) / running_max
    zeros = pd.Series(0.0, index=value.index)
    return {
        "value": value,
        "drawdown": drawdown,
        "income": zeros.copy(),
        "withdrawn": zeros.copy(),
        "withdrawn_daily": zeros.copy(),
        "target_daily": zeros.copy(),
        "wealth": value.copy(),
        # The tracker total-return index already incorporates distributions.
        # Do not invent a separate spendable-income series from that index.
        "total_income": 0.0,
        "total_income_gross": 0.0,
        "total_tax": 0.0,
        "total_withdrawn": 0.0,
        "reinvested_surplus": 0.0,
        "final_value": float(value.iloc[-1]),
        "final_cash": 0.0,
        "depleted": None,
    }
...
def run_backtest(portfolios: List[dict], benchmark: Optional[str],
    ...
    portfolios = [
      {name, source: "hypothetical", holdings: [{ticker, weight}]},
      {name, source: "actual", history: {dates, values}},
    ]
    ...
    actual_count = 0
    for p in portfolios:
        source = str(p.get("source") or "hypothetical").strip().lower()
        if source not in {"hypothetical", "actual"}:
            raise ValueError(f"Portfolio '{p.get('name','?')}' has an invalid source.")
        if source == "actual":
            actual_count += 1
            ...
            # Validate now so bad server history fails before any market download.
            simulate_actual_history(p.get("history") or {}, initial)
            continue
    ...
    if actual_count > 1:
        raise ValueError("Choose actual transaction-aware history for only one portfolio.")
    ...
    all_tickers = set()
    for p in portfolios:
        if str(p.get("source") or "hypothetical").strip().lower() == "actual":
            continue
        for h in p["holdings"]:
    ...
    if all_tickers:
        ...
        close, divs = pd.DataFrame(), pd.DataFrame()
        ok, bad = [], []
    ...
    for p in portfolios:
        source = str(p.get("source") or "hypothetical").strip().lower()
        if source == "actual":
            sim = simulate_actual_history(p.get("history") or {}, initial)
        else:
        ...
        results.append({
            ...
            "history": p.get("history") if source == "actual" else None,
        })
    ...
    for r in results:
        ...
        series_out.append({
            ...
            "source": r["source"],
            "holdings": r["holdings"],
            "actual_start_date": (r.get("history") or {}).get("actual_start_date"),
            "actual_end_date": (r.get("history") or {}).get("actual_end_date"),
            "tracker_metrics": (r.get("history") or {}).get("metrics"),
            "metrics": metrics,
