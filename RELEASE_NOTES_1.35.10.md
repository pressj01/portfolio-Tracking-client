# Portfolio Tracking Client v1.35.10
...
- Fixed overlapping Snowball and Fidelity transaction imports creating duplicate DRIP purchases when both feeds reported the same ticker, date, and fractional shares with slightly different effective prices. The broader comparison applies only to marked reinvestments; ordinary BUY and SELL fills retain strict price matching, and legitimate repeated same-day transactions are still preserved by occurrence count.

## Transaction Table

- Added inline **up/down arrows** beside each eligible same-day trade so the broker's execution order can be customized directly in the expanded holdings ledger. The controls recalculate FIFO cost basis immediately and correctly target the owning portfolio when viewed through an aggregate.
- Kept the ticker header and expanded transaction column headers opaque and readable while scrolling, and replaced ambiguous dashes in the Same-day Order column with **Only trade** or **N/A**.
...
**Full Changelog**: https://github.com/pressj01/portfolio-Tracking-client/compare/v1.35.9...v1.35.10
