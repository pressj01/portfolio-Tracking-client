...
function SummaryStrip({ model, monthlyExpenses, bufferRatio, targetCashMonths }) {
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.6rem', marginBottom: '1rem' }}>
      ...
      <StatTile label="Bear Market After Tax" value={fmtMoney(model.bearIncomeProtected)}
        color={model.bearIncomeProtected >= model.expensesPortfolioMustPay ? '#f9d66d' : '#ff6b6b'} />
      ...
      <StatTile label="LEPB Target" value={fmtMoney(model.bufferTargetNow)}
        sub={`${bufferRatio.toFixed(2)}x portfolio-paid expenses`} color="#7ecfff" />
      <StatTile label="Bear Buffer Ratio"
        value={Number.isFinite(model.bearBufferRatio) ? `${model.bearBufferRatio.toFixed(2)}x` : 'Covered'}
        color={model.bearBufferRatio >= bufferRatio ? '#00e89a' : model.bearBufferRatio >= 1 ? '#f9d66d' : '#ff6b6b'} />
      <StatTile label="Buffer Gap" value={fmtMoney(model.bufferGap)}
        color={model.bufferGap <= 0 ? '#00e89a' : '#ffb74d'} />
      <StatTile label="Cash Target" value={fmtMoney(model.targetCash)} sub={`${targetCashMonths} months`} />
      <StatTile label="Cash Runway"
        value={Number.isFinite(model.cashRunwayMonths) ? `${model.cashRunwayMonths.toFixed(1)} mo` : 'Covered'}
        color={model.cashRunwayMonths < 12 ? '#ff6b6b' : '#00e89a'} />
    </div>
  )
}

function CalculationPanels({ model, bufferRatio }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1rem', marginBottom: '1rem' }}>
      <div style={panelStyle}>
        ...
        <MetricLine label="After Tax and Minimum Reinvestment" value={fmtMoney(model.targetIncomeAfterMinimumReinvest)}
          tone={model.targetIncomeAfterMinimumReinvest >= 0 ? 'good' : 'bad'} />
        <MetricLine label="Bear-Market Income After Tax" value={fmtMoney(model.bearIncomeProtected)}
          tone={model.bearIncomeProtected >= model.expensesPortfolioMustPay ? 'good' : 'bad'} />
      </div>

      <div style={panelStyle}>
        <h3 style={panelTitleStyle}>Important Monthly Metrics</h3>
        <MetricLine label="Income Needed to Hit LEPB" value={fmtMoney(model.bufferTargetNow)} />
        <MetricLine label="Income Over / Under LEPB" value={fmtMoney(model.incomeOverUnderTarget)}
          tone={model.incomeOverUnderTarget >= 0 ? 'good' : 'bad'} />
        <MetricLine label="Good-Market Buffer Ratio"
          value={Number.isFinite(model.goodBufferRatio) ? `${model.goodBufferRatio.toFixed(2)}x` : 'Covered'}
          tone={model.goodBufferRatio >= bufferRatio ? 'good' : 'warn'} />
        <MetricLine label="Bear-Market Buffer Ratio"
          value={Number.isFinite(model.bearBufferRatio) ? `${model.bearBufferRatio.toFixed(2)}x` : 'Covered'}
          tone={model.bearBufferRatio >= bufferRatio ? 'good' : model.bearBufferRatio >= 1 ? 'warn' : 'bad'} />
        ...
        <MetricLine label="Excess After Expenses & Reinvest Good" value={fmtMoney(model.excessAfterExpensesAndReinvestGood)}
          tone={model.excessAfterExpensesAndReinvestGood >= 0 ? 'good' : 'bad'} />
        <MetricLine label="Excess After Expenses & Reinvest Bear" value={fmtMoney(model.excessAfterExpensesAndReinvestBear)}
          tone={model.excessAfterExpensesAndReinvestBear >= 0 ? 'good' : 'bad'} />
...
function LepbTrendChart({ chartData }) {
  return (
    <div style={panelStyle}>
      <h3 style={panelTitleStyle}>Passive Income - LEPB Trend Lines</h3>
      <Plot
        data={chartData}
        layout={{
          ...
          legend: { orientation: 'h', y: 1.2, x: 0.5, xanchor: 'center', yanchor: 'bottom', font: { color: '#ccc' } },
          hovermode: 'x unified',
...
function MilestonesPanel({ model, monthLabel }) {
  return (
    <div style={panelStyle}>
      <h3 style={panelTitleStyle}>Milestones</h3>
      <div style={milestoneStyle}><span>Income covers expenses</span><strong>{monthLabel(model.firstExpenseMonth)}</strong></div>
      <div style={milestoneStyle}><span>Income reaches LEPB target</span><strong>{monthLabel(model.firstBufferMonth)}</strong></div>
      ...
      <div style={milestoneStyle}><span>Final monthly income</span><strong>{fmtMoney(model.finalIncome)}</strong></div>
      <div style={milestoneStyle}><span>Final monthly expenses</span><strong>{fmtMoney(model.finalExpenses)}</strong></div>
      <div style={milestoneStyle}><span>Final cash reserve</span><strong>{fmtMoney(model.finalCash)}</strong></div>
    </div>
  )
}

function YearlyProjectionTable({ yearlyRows, bufferRatio }) {
  return (
    <div style={panelStyle}>
        ...
        <table style={{ fontSize: '0.8rem' }}>
          <thead>
            <tr>
              ...
              <th style={{ textAlign: 'right' }}>LEPB Target</th>
          ...
          <tbody>
            {yearlyRows.map(r => (
              <tr key={r.year}>
                ...
                <td style={{ textAlign: 'right', color: r.coverage >= bufferRatio ? '#00e89a' : r.coverage >= 1 ? '#f9d66d' : '#ff6b6b' }}>{Number.isFinite(r.coverage) ? r.coverage.toFixed(2) + 'x' : 'Covered'}</td>
                <td style={{ textAlign: 'right', color: r.bearCoverage >= bufferRatio ? '#00e89a' : r.bearCoverage >= 1 ? '#f9d66d' : '#ff6b6b' }}>{Number.isFinite(r.bearCoverage) ? r.bearCoverage.toFixed(2) + 'x' : 'Covered'}</td>
...
function MonthlyLepbTable({ projectionRows, bufferRatio, directContribution }) {
  return (
    <div style={{ ...panelStyle, marginTop: '1rem' }}>
      <h3 style={panelTitleStyle}>Monthly LEPB Projection Table</h3>
        ...
        <table style={{ fontSize: '0.74rem', minWidth: 1780 }}>
          <thead>
            ...
            <tr>
              ...
              <th style={stickyHeadStyle}>LEPB Target</th>
              <th style={stickyHeadStyle}>LEPB Ratio</th>
...
function TopIncomeSources({ topHoldings }) {
  return (
    <div style={{ ...panelStyle, marginTop: '1rem' }}>
...
const INPUT_HELP = {
  ...
  bufferRatio: 'Living Expense Protection Buffer multiplier. A ratio of 3 means stressed income should be three times monthly expenses.',
...
export default function RetirementReadiness() {
  ...
  const model = useMemo(() => {
    ...
    for (let i = 1; i <= months; i += 1) {
      ...
      series.push({
        month: i, label: `M${i}`, year: Math.ceil(i / 12),
        income: goodAfterTax, incomeBeforeTax: goodBeforeTax,
        bearIncome: bearAfterTax, bearIncomeBeforeTax: bearBeforeTax,
        expenses, nonInvestment, netExpenses, target,
        excessAfterExpensesGood: goodAfterTax - netExpenses,
        excessAfterExpensesBear: bearAfterTax - netExpenses,
        surplus, shortfall, minReinvest, reinvested,
        bearReinvested: Math.min(Math.max(0, bearAfterTax), minReinvest),
        withdrawnToCash, cash, bookNav, currentNav, bearBookNav, bearCurrentNav,
        coverage, bearCoverage, targetCoverage,
      })
    }
    ...
    const avgAnnual = (key) => first36.length ? first36.reduce((s, r) => s + (r[key] || 0), 0) / first36.length * 12 : 0
    const avgMonthly = (key) => first36.length ? first36.reduce((s, r) => s + (r[key] || 0), 0) / first36.length : 0
    const avgMonthlyReinvested = avgMonthly('reinvested')
    const avgMonthlyGoodIncome = avgMonthly('income')

    return {
      portfolioValue, navBase, bearNav, annualIncome, currentMonthlyIncome,
      nonInvestmentGross, nonInvestmentAfterTax, expensesPortfolioMustPay,
      targetIncomeBeforeTax, targetIncomeAfterTax, targetIncomeAfterMinimumReinvest,
      bearIncomeBeforeTax, bearIncomeAfterTax, bearIncomeProtected, bearIncomeAfterMinimumReinvest,
      protectedMonthlyIncome: bearIncomeProtected, currentYield, targetYield, bearYield,
      targetCash, minReinvestForErosion, minReinvestForInflation, minimumMonthlyReinvest,
      bufferTargetNow, currentCoverage, goodBufferRatio, bearBufferRatio, protectedCoverage,
      bufferGap, shortfallNow, cashRunwayMonths, incomeOverUnderTarget,
      excessAfterExpensesAndReinvestGood, excessAfterExpensesAndReinvestBear,
      avgAnnualWithdrawals: avgAnnual('withdrawnToCash') + avgAnnual('netExpenses'),
      avgAnnualIncomeGood: avgAnnual('income'), avgAnnualIncomeBear: avgAnnual('bearIncome'),
      ...
      firstBufferMonth, firstExpenseMonth, depletedMonth,
    ...
    holdings, monthlyExpenses, bufferRatio, surplusWithdrawPct, surplusReinvestPct,
    startingCash, targetCashMonths, years, inflationPct,
    employmentIncome, companyPension, govPension, annuities, otherRecurringIncome,
    incomeIndexPct, nonInvestmentTaxPct, portfolioBookNav, targetYieldPct,
    navErosionPct, bearDeclinePct, bearYieldPct, investmentTaxPct, directContribution, incomeHaircutPct,
  ])

  const chartData = useMemo(() => {
    const s = model.series
    return [
      { x: s.map(r => r.month), y: s.map(r => r.expenses), type: 'scatter', mode: 'lines', name: 'Avg monthly expenses', line: { color: '#b8d8ea', width: 2, dash: 'dash' }, hovertemplate: 'Month %{x}<br>Expenses: $%{y:,.2f}<extra></extra>' },
      { x: s.map(r => r.month), y: s.map(r => r.netExpenses), type: 'scatter', mode: 'lines', name: 'Net exp after inflows', line: { color: '#315f83', width: 2, dash: 'dot' }, hovertemplate: 'Month %{x}<br>Net expenses: $%{y:,.2f}<extra></extra>' },
      { x: s.map(r => r.month), y: s.map(r => r.bearIncome), type: 'scatter', mode: 'lines', name: 'Dist. @ bear yield after tax', line: { color: '#ff6b6b', width: 3 }, hovertemplate: 'Month %{x}<br>Bear income: $%{y:,.2f}<extra></extra>' },
      { x: s.map(r => r.month), y: s.map(r => r.income), type: 'scatter', mode: 'lines', name: 'Dist. @ target yield after tax', line: { color: '#39c686', width: 3 }, hovertemplate: 'Month %{x}<br>Good income: $%{y:,.2f}<extra></extra>' },
      { x: s.map(r => r.month), y: s.map(r => r.target), type: 'scatter', mode: 'lines', name: 'Target LEPB $', line: { color: '#c57aa8', width: 2, dash: 'dash' }, hovertemplate: 'Month %{x}<br>Target: $%{y:,.2f}<extra></extra>' },
    ]
  }, [model.series])
  ...
  const yearlyRows = useMemo(() => {
    ...
    for (let y = 1; y <= Math.ceil(model.series.length / 12); y += 1) {
      ...
      rowsByYear.push({
        year: y, income: last.income, bearIncome: last.bearIncome,
        expenses: last.expenses, nonInvestment: last.nonInvestment,
        netExpenses: last.netExpenses, target: last.target,
        coverage: last.coverage, bearCoverage: last.bearCoverage,
        ...
        cash: last.cash, bookNav: last.bookNav, currentNav: last.currentNav,
      })
    }
    return rowsByYear
  }, [model.series])
  ...
  const projectionRows = useMemo(() => {
    ...
    for (let y = 1; y <= Math.ceil(model.series.length / 12); y += 1) {
      ...
      slice.forEach(r => {
        ...
        rows.push({ type: 'month', key: `m-${r.month}`, year: r.year, monthLabel: `${monthNames[d.getMonth()]}-${String(d.getFullYear()).slice(-2)}`, ...r })
      })
      if (slice.length) {
        const last = slice[slice.length - 1]
        const sum = key => slice.reduce((s, r) => s + (Number(r[key]) || 0), 0)
        rows.push({
          type: 'total', key: `t-${y}`, year: y, monthLabel: 'TOTALS',
          expenses: sum('expenses'), nonInvestment: sum('nonInvestment'),
          netExpenses: sum('netExpenses'), bookNav: last.bookNav, currentNav: last.currentNav,
          incomeBeforeTax: sum('incomeBeforeTax'), income: sum('income'), target: sum('target'),
          coverage: last.coverage, excessAfterExpensesGood: sum('excessAfterExpensesGood'),
          minReinvest: sum('minReinvest'), reinvested: sum('reinvested'),
          withdrawnToCash: sum('withdrawnToCash'), bearBookNav: last.bearBookNav,
          bearCurrentNav: last.bearCurrentNav, bearIncomeBeforeTax: sum('bearIncomeBeforeTax'),
          bearIncome: sum('bearIncome'), bearCoverage: last.bearCoverage,
          excessAfterExpensesBear: sum('excessAfterExpensesBear'), bearReinvested: sum('bearReinvested'),
        })
      }
    }
    return rows
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '1rem', marginBottom: '0.9rem' }}>
        <div>
          ...
          <p style={{ color: '#90a4ae', fontSize: '0.84rem' }}>
            {currentProfileName}{isAggregate ? ' aggregate' : ''} income measured against monthly expenses and a living-expense protection buffer.
          </p>
        </div>
      ...
      {!loading && !error && (
        <>
          <InputSection title="Critical Monthly Inputs">
            ...
            <InputField label="LEPB Ratio" help={INPUT_HELP.bufferRatio}>
          ...
          <SummaryStrip model={model} monthlyExpenses={monthlyExpenses} bufferRatio={bufferRatio} targetCashMonths={targetCashMonths} />

          <CalculationPanels model={model} bufferRatio={bufferRatio} />

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1rem', marginBottom: '1rem' }}>
            <LepbTrendChart chartData={chartData} />
            <MilestonesPanel model={model} monthLabel={monthLabel} />
          </div>

          <YearlyProjectionTable yearlyRows={yearlyRows} bufferRatio={bufferRatio} />

          <MonthlyLepbTable projectionRows={projectionRows} bufferRatio={bufferRatio} directContribution={directContribution} />

          <TopIncomeSources topHoldings={topHoldings} />
        </>
      )}
    </div>
  )
...
const fieldStyle = {
  ...
  color: '#9fb0c0', fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase',
}

const sectionStyle = {
  background: '#111124', border: '1px solid #2a2a3e', borderRadius: 8,
  padding: '0.85rem', marginBottom: '1rem',
}

const sectionTitleStyle = {
  color: '#7ecfff', fontSize: '0.92rem', margin: '0 0 0.75rem',
}
...
const inputStyle = {
  width: '100%', minHeight: 34, background: '#1a1a2e',
  border: '1px solid #3a3a5c', borderRadius: 5,
  color: '#e0e8f5', padding: '0.35rem 0.5rem', fontSize: '0.86rem',
}

const metricLineStyle = {
  display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: '1rem',
  borderBottom: '1px solid #222a40', padding: '0.48rem 0', color: '#aab7c4', fontSize: '0.82rem',
}

const stickyHeadStyle = {
  position: 'sticky', top: 0, zIndex: 1, background: '#172035',
  color: '#d5e8f6', whiteSpace: 'nowrap', textAlign: 'center',
}
...
const totalRowStyle = {
  background: 'rgba(0, 232, 154, 0.12)', color: '#a8f0d0', fontWeight: 700,
}

const panelStyle = {
  background: '#111124', border: '1px solid #2a2a3e', borderRadius: 8, padding: '0.85rem',
}

const panelTitleStyle = {
  margin: '0 0 0.75rem', color: '#7ecfff', fontSize: '0.98rem',
}

const milestoneStyle = {
  display: 'flex', justifyContent: 'space-between', gap: '1rem',
  borderBottom: '1px solid #222a40', padding: '0.55rem 0', color: '#aab7c4', fontSize: '0.82rem',
}
