    ...
    <div className="page">
      ...
      <div className="card" style={{ padding: '0.75rem 1rem', marginBottom: '1rem' }}>
        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap', marginBottom: '0.5rem' }}>
          ...
          <span style={{ color: 'var(--p-556677)', margin: '0 0.3rem' }}>|</span>

          <span style={{ color: 'var(--text-dim)', fontSize: '0.82rem', marginRight: '0.3rem' }}>Period:</span>
          {PERIODS.map(p => (
            <button
              key={p.value}
        ...
        <>
          ...
          <div className="card" style={{ padding: '0.75rem 1rem', marginBottom: '1rem', overflowX: 'auto' }}>
            ...
            <table style={{ borderCollapse: 'collapse', fontSize: '0.82rem', width: 'auto' }}>
              <thead>
                <tr>
                  <th style={{ padding: '0.4rem 0.6rem', borderBottom: '1px solid var(--p-2a3a4e)', color: 'var(--text-dim)' }}></th>
                  {result.tickers.map(t => (
                    <th key={t} style={{
                      padding: '0.4rem 0.6rem', borderBottom: '1px solid var(--p-2a3a4e)',
                      color: 'var(--accent-bright)', fontWeight: 600, textAlign: 'center', minWidth: 55,
                    }}>{t}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {result.tickers.map((rowT, i) => (
                  <tr key={rowT}>
                    <td style={{
                      padding: '0.4rem 0.6rem', fontWeight: 600, color: 'var(--accent-bright)',
                      borderRight: '1px solid var(--p-2a3a4e)', whiteSpace: 'nowrap',
                    }}>{rowT}</td>
                    {result.matrix[i].map((v, j) => {
                      ...
                      return (
                        <td key={j} style={{
                          ...
                          background: isDiag ? 'var(--p-2a3a4e)' : corrColor(v),
                          color: isDiag ? 'var(--p-aaa)' : corrTextColor(v),
