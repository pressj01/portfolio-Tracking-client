...
function ThresholdEditor({ criterion, thresholds, onChange }) {
  ...
  if (key === 'expense') {
    ...
    controls = (
      <>
        <div>
          <label style={labelStyle}>Flag if beta &gt;</label>
          <input type="number" step="0.1" style={inputStyle} value={t.highBeta}
            onChange={e => update({ highBeta: Number(e.target.value) })} />
        </div>
        <div>
          <label style={labelStyle}>Flag if beta &lt;</label>
          <input type="number" step="0.1" style={inputStyle} value={t.lowBeta}
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem 1.5rem', marginTop: '0.6rem', color: '#b8c8e0', fontSize: '0.9rem' }}>
        ...
        <span><span style={{ color: '#90a4ae' }}>NAV: </span><strong style={{ color: '#e6edf7' }}>{fund.nav != null ? `$${Number(fund.nav).toFixed(2)}` : '-'}</strong></span>
...
export default function ETFBuyingChecklistEvaluator() {
  ...
  return (
    <div className="page cef-page">
      ...
      {isOptionIncome && fund && (
        <div style={{
          background: '#1f2e52', border: '1px solid #2a3e6b', borderRadius: 6,
          padding: '0.8rem 1rem', marginBottom: '1rem', color: '#8aa0c8', fontSize: '0.92rem',
        }}>
          <strong style={{ color: '#ffd76a' }}>{fund.ticker}</strong> is classified as an
          option-income / derivative-income ETF.
          The <Link to="/option-income-etf-evaluator" style={{ color: '#58c4d8' }}>Option-Income ETF Evaluator</Link> uses
