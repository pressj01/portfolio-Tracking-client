...
const SCANNER_TRADE_PLANS = [
  ...
  },
  {
    ...
    vega: 'mixed',
    diagram: {
      points: [[0, -1.1], [2.2, -1.1], [3.2, 1], [5.8, 1], [6.8, 0.1], [10, 0.1]],
      strikeMarkers: [{ x: 2.2, label: 'LL' }, { x: 3.2, label: 'LS' }, { x: 5.8, label: 'US' }, { x: 6.8, label: 'UL' }],
      centerLabel: 'Spot near upper spread',
    },
    howItWorks: 'Choose a Put Condor, a mirrored Call Condor, or Both. Each side buys a near-the-money option and sells the option one point farther out, then finances that debit spread with a wider credit spread. Each complete four-leg package must open for a small credit on its untested expiration side.',
    entry: 'Choose Mini-SPX (^XSP) or SPY, at-the-money or slightly out-of-the-money debit placement, an expiration window, a 10–20 delta target for each credit-spread short option, and a hard per-condor dollar maximum risk. The scanner automatically chooses the farther protective long and width without exceeding that limit.',
    profit: 'Each four-leg result shows its own maximum profit and loss. Both mode also shows the true combined eight-leg expiration maximum profit, maximum loss, and downside, middle, and upside outcomes; it does not simply add two peaks that occur at different prices.',
    hold: 'The default scan centers near 42 DTE. Reassess around halfway and two-thirds through the original cycle, and avoid carrying the four-leg position into concentrated late-expiration gamma unless that is part of a separately tested plan.',
    adjustment: 'Reprice and manage each four-leg side as a complete package. Do not widen a credit spread or add contracts after entry merely to collect more credit, because either change overrides the maximum-risk choice that defined the trade.',
    exit: 'Close when a complete-position loss reaches its predeclared limit, the underlying moves through the applicable risk breakeven, execution quality deteriorates, or the original thesis fails. The displayed risk is an expiration payoff calculation, not a guarantee against interim marks, margin treatment, assignment, or fees.',
  },
  {
  ...
  },
  {
    ...
    profit: 'The course uses a $1,000 management figure, scaled with quantity. It is an editable planning reference, not a predicted result. If price enters the tent with a worthwhile gain before that reference, closing early is valid; do not wait for the body or expiration peak.',
    hold: 'The course’s 16-week (112-day) calendar estimate is a review horizon, not a predicted outcome. Close earlier when the pre-entered exit rule or risk limit calls for it.',
  ...
  },
  {
    ...
    entry: 'Use roughly 160–230 DTE, centered near 200 DTE, with guide deltas near 25/15/2.5. Spot should begin on the upper flat line with the long-dated tent below it, commonly about 6%–10% away. Enter only when price, concavity, and skew signals are favorable; four or five warnings means no entry. Budget about $12,500 at the course base size. The scanner also offers a 30/12/3-delta plan near 100 DTE. You set its opening debit or credit and its lower longs shift up or down to fit (the upper long moves down only when they cannot), and it is not held to these CC4 entry rules. Either plan scales from 1 / −2 / +2 upward.',
    profit: 'The course uses $1,000 and $800 as scaled planning references. They are not expected results. If price enters the tent with a worthwhile gain, closing before the body is a valid exit decision.',
    hold: 'The course’s 12-week (84-day) calendar estimate is a review horizon, not a predicted outcome. A position may close sooner when its pre-entered exit rule or risk limit applies.',
  ...
  },
  {
    ...
    profit: 'The source material uses a 7%–15% management band on capital at risk. Treat it as an editable exit-planning input, not an expected return. If price enters the tent with a satisfactory gain, consider closing rather than waiting for the body. The preferred review window is halfway to two-thirds through the cycle; the latest planned exit is 15–20 DTE.',
  ...
  },
  {
    ...
    profit: 'Review modeled P&L around days 8 and 14. The presentation examples are illustrations, not targets or predicted results. Use a pre-entered dollar exit threshold that fits the position and risk plan rather than waiting for the tent peak.',
  ...
  {
    id: 'fourteen-day-aic-scanner',
    name: '14-Day Asymmetrical Iron Condor Scanner',
    family: 'Hedged short-hold iron condor',
    route: generalScannerRoute('fourteen-day-aic'),
    outlook: 'neutral',
    vega: 'short',
    diagram: {
      points: [[0, -0.4], [1.6, -0.15], [3.2, 0.9], [6.4, 0.9], [8.2, -1.2], [10, -1.2]],
      strikeMarkers: [{ x: 1.6, label: 'P long' }, { x: 3.2, label: 'P short' }, { x: 6.4, label: 'C short' }, { x: 8.2, label: 'C long' }],
      centerLabel: 'Slightly long T+0',
    },
    howItWorks: 'Sell more put credit spreads than call credit spreads and buy a closer put debit spread as a downside hedge. The result is a defined-risk condor with a flatter T+0 line, slightly long delta, and less upside risk than a balanced iron condor.',
    entry: 'Use IWM, SPY, QQQ, or VOO at 30–35 DTE. Default unit is 4 put credits, 1 call credit (~12 delta), and 1 put debit hedge. Short put ~25 delta. Plan capital is about $18,000 per unit.',
    profit: 'The source video uses a 2–4% management band on plan capital. It is an editable exit input, not an expected return. Review the complete position against that rule rather than holding leftover risk.',
    hold: 'Maximum 14 days in the trade. That hold is why it is called the 14-day AIC — it is not a 14-DTE option.',
    adjustment: 'Keep the structure slightly long and defined-risk. The debit hedge is already on; do not wait to add downside protection. Prefer reducing or exiting over adding extra pieces in a two-week hold.',
    exit: 'Exit when the pre-entered management threshold, a 5% plan-capital loss, or the 14-day hold backstop applies, whichever comes first.',
  },
  {
    id: 'monthly-aic-scanner',
    name: 'Monthly Asymmetrical Iron Condor Scanner',
    family: 'Hedged monthly iron condor',
    route: generalScannerRoute('monthly-aic'),
    outlook: 'neutral',
    vega: 'short',
    diagram: {
      points: [[0, -0.35], [1.8, -0.1], [3.4, 1.1], [6.6, 1.1], [8.4, -1.15], [10, -1.15]],
      strikeMarkers: [{ x: 1.8, label: 'P long' }, { x: 3.4, label: 'P short' }, { x: 6.6, label: 'C short' }, { x: 8.4, label: 'C long' }],
      centerLabel: 'Slightly long T+0',
    },
    howItWorks: 'Same AIC pieces as the 14-day trade — put-heavy credits, smaller call credits, put debit hedge — entered farther from expiration so there is more time to absorb adjustments.',
    entry: 'Enter 40–50 DTE on IWM, SPY, QQQ, or VOO. Default unit is 10 put credits, 2 call credits (~12 delta), and 1 put debit hedge. Short put ~16 delta. Same ~$18,000 plan capital.',
    profit: 'The source material uses a 7–8% management band on plan capital. It is an editable exit input, not an expected return; do not wait for expiration solely to pursue it.',
    hold: 'About 30 days on average. The planned backstop is 14 DTE remaining so the next monthly cycle does not overlap.',
    adjustment: 'The debit hedge is already on. Manage to keep delta slightly long and upside risk small. The extra time versus the 14-day campaign is what makes adjustments cheaper to recover.',
    exit: 'Exit when the pre-entered management threshold, a 5% plan-capital loss, or 14 DTE remaining applies, whichever comes first.',
  },
]
