...
function GeneralOptionScannerWorkspace({ initialStrategy }) {
  ...
  const summaryGroups = useMemo(() => {
    ...
    { title: 'Fundamental data', help: 'Filters individual stocks using the app\'s Fundamental and Growth scores. ETFs do not require these company-level scores.', items: [
      { label: 'Stock Score Fundamental', value: scoreText(filters, 'fundamental'), help: 'A transparent 1–10 app score for company quality and value. It averages valuation (forward or trailing P/E), profit margin, return on equity, current ratio, and lower debt-to-equity. Higher is better; missing inputs are excluded. It applies only to individual stocks; ETFs and index funds such as SPY, QQQ, and IWM skip this filter.', editor: <ScoreRange label="Quality and value" name="fundamental" filters={filters} setFilter={setFilter} /> },
    ...
    { title: 'Consolidated options data', help: 'Filters the option chain by total trading volume and IV Rank, helping focus scans on liquid or volatility-specific opportunities.', items: [
