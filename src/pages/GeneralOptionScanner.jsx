...
function earningsInTradeText(mode) {
  if (mode === 'skip') return 'Skip'
  if (mode === 'require') return 'Require'
  return 'Allow'
}

function matchResultsText(includeNearMatches) {
  return includeNearMatches ? 'Nearest if none qualify' : 'Exact only'
}
  ...
  return <div className="gos-table-wrap"><table className="gos-table">
    ...
    <tbody>{rows.map((row, index) => {
      ...
      return <tr key={key} className={`${active ? 'selected ' : ''}${nearMatch ? 'near-match' : unverified ? 'unverified-match' : ''}`.trim()} onClick={() => setSelected(row)} title={title}>
        ...
        <td title={`25-delta put IV minus ATM put IV. High means OTM puts are unusually expensive. Current gap: ${signedPoints(meta.put_skew)} volatility points.`}>{rankCell(meta.put_skew_rank, meta.put_skew_rank_observations, 'Put Skew Rank needs about 20 daily observations', !meta.put_skew_rank_ready)}</td>
        <td title={`25-delta call IV minus ATM call IV. High means OTM calls are unusually expensive. Current gap: ${signedPoints(meta.call_skew)} volatility points.`}>{rankCell(meta.call_skew_rank, meta.call_skew_rank_observations, 'Call Skew Rank needs about 20 daily observations', !meta.call_skew_rank_ready)}</td>
...
function GeneralOptionScannerWorkspace({ initialStrategy }) {
  ...
  const [filters, setFilters] = useState(() => {
    ...
    const restoredFilters = normalizeEarningsFilter({
      ...defaultsForGeneralStrategy(initialStrategy),
      ...(restored?.filters || {}),
    })
  ...
  const summaryGroups = useMemo(() => {
    ...
    { title: 'Consolidated options data', help: 'Filters the option chain by total trading volume, IV Rank, IV−RV, RV Rank, Volatility score, and skew rank.', items: [
      ...
      { label: 'Put Skew Rank', value: rangeText(filters.min_put_skew_rank ?? 0, filters.max_put_skew_rank ?? 100), help: 'Ranks the roughly 30-DTE 25-delta put IV minus ATM put IV against this ticker’s own trailing year. High values mean out-of-the-money puts are unusually expensive. Put-selling and put-spread presets require a high reading. Missing history does not hide a trade.', editor: <div className="gos-quick-pair"><DynamicField field={{ key: 'min_put_skew_rank', label: 'Minimum Put Skew Rank', suffix: '%', step: 5, min: 0, max: 100 }} value={filters.min_put_skew_rank ?? 0} onChange={value => setFilter('min_put_skew_rank', Math.min(value, filters.max_put_skew_rank ?? 100))} /><DynamicField field={{ key: 'max_put_skew_rank', label: 'Maximum Put Skew Rank', suffix: '%', step: 5, min: 0, max: 100 }} value={filters.max_put_skew_rank ?? 100} onChange={value => setFilter('max_put_skew_rank', Math.max(value, filters.min_put_skew_rank ?? 0))} /></div> },
      { label: 'Call Skew Rank', value: rangeText(filters.min_call_skew_rank ?? 0, filters.max_call_skew_rank ?? 100), help: 'Ranks the roughly 30-DTE 25-delta call IV minus ATM call IV against this ticker’s own trailing year. High values mean out-of-the-money calls are unusually expensive. Call-selling and call-spread presets require a high reading. Missing history does not hide a trade.', editor: <div className="gos-quick-pair"><DynamicField field={{ key: 'min_call_skew_rank', label: 'Minimum Call Skew Rank', suffix: '%', step: 5, min: 0, max: 100 }} value={filters.min_call_skew_rank ?? 0} onChange={value => setFilter('min_call_skew_rank', Math.min(value, filters.max_call_skew_rank ?? 100))} /><DynamicField field={{ key: 'max_call_skew_rank', label: 'Maximum Call Skew Rank', suffix: '%', step: 5, min: 0, max: 100 }} value={filters.max_call_skew_rank ?? 100} onChange={value => setFilter('max_call_skew_rank', Math.max(value, filters.min_call_skew_rank ?? 0))} /></div> },
      { label: 'Skew Rank', value: rangeText(filters.min_skew_rank, filters.max_skew_rank), help: 'Ranks the roughly 30-DTE 25-delta put-IV minus 25-delta call-IV gap against this ticker’s own trailing year. High values mean puts are unusually expensive versus calls; low values mean calls are unusually expensive. Put-selling presets prefer a high reading; call-selling presets prefer a low reading. Missing history does not hide a trade.', editor: <div className="gos-quick-pair"><DynamicField field={{ key: 'min_skew_rank', label: 'Minimum Skew Rank', suffix: '%', step: 5, min: 0, max: 100 }} value={filters.min_skew_rank} onChange={value => setFilter('min_skew_rank', Math.min(value, filters.max_skew_rank))} /><DynamicField field={{ key: 'max_skew_rank', label: 'Maximum Skew Rank', suffix: '%', step: 5, min: 0, max: 100 }} value={filters.max_skew_rank} onChange={value => setFilter('max_skew_rank', Math.max(value, filters.min_skew_rank))} /></div> },
    ] },
    ...
    <div className="scanner-filter-workspace">
      <CompactScannerFilterPanel title={scanner?.label || 'Choose a scan'} strategyControl={strategyPicker} groups={summaryGroups} onRun={runScan} loading={loading} disabled={!strategy} toolbar={strategy && (
        <div className="gos-preset-bar">
          <span>{strategy === DOUBLE_HEDGE_STRATEGY ? 'Risk / starting point' : 'Starting point'}</span>
          ...
          {strategyFields.filter(field => field.toolbar).map(field => <Fragment key={field.key}>
            ...
            {field.toolbar === 'buttons'
              ...
              : field.toolbar === 'number'
                ? <label className="gos-preset-number" title={helpForGeneralField(field)}>
                  ...
                  <input type="number" aria-label={field.label} min={field.min} step={field.step} disabled={field.disabledWhen?.(filters)} value={filters[field.key] ?? ''} onChange={event => setFilter(field.key, event.target.value === '' ? null : Number(event.target.value))} />
                </label>
            ...
            {field.statusText && <small className="gos-preset-note">{field.statusText(filters)}</small>}
          </Fragment>)}
