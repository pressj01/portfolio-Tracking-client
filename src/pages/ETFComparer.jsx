...
export default function ETFComparer() {
  ...
  const chart = useMemo(() => {
    ...
    const baseTitle = titleWindow
      ...
      : (isActualPrice ? `${returnModeLabel} ($)` : `${returnModeLabel} (%)`)
    ...
    return {
      ...
      layout: {
        ...
        yaxis: {
          ...
          tickformat: logScaleActive && !isActualPrice ? ',.0f' : ',.2f',
          gridcolor: '#333',
          zerolinecolor: '#555',
          showspikes: true,
          spikemode: 'across',
      ...
      <div className="etfc-toolbar">
        ...
        <div className="etfc-periods">
          <select className="etfc-select" value={returnMode} onChange={e => setReturnMode(e.target.value)}>
            {RETURN_MODES.map(m => (
              <option key={m.value} value={m.value} title={m.value === 'actual' ? "Chart each ETF's actual share price in dollars, not a relative return" : undefined}>
                {m.label}
              </option>
            ))}
          </select>
        ...
        <div className="etfc-mode-bar">
          {RETURN_MODES.map(m => (
            <button
              ...
              title={m.value === 'actual' ? "Chart each ETF's actual share price in dollars, not a relative return" : undefined}
            >{m.label}</button>
          ))}
        </div>
