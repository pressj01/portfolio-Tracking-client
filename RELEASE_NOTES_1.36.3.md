# Portfolio Tracking Client v1.36.3

This release adds the changes made since the September 24 v1.36.2 deployment to the portfolio tracking, dividend, broker-import, research, and option-scanner improvements described below.

Desktop installers are available for Windows PC, Intel Mac, and Apple-silicon Mac.

## New since v1.36.2

### Account Change with Money-Market Funds

- Money-market funds such as FZDXX, FDRXX, SPAXX, and FSPXX now count as unchanged in Account Change instead of as missing prices. Yahoo publishes only the current $1.00 price for these funds, with no previous close. A portfolio with a large money-market balance showed "Unavailable — Yahoo has no … close," and so did an Owner or combined view that included it, while a portfolio holding less of the same fund still showed a figure.
- Other holdings missing a close are still left out and named, and Account Change still withholds a figure when too much of the portfolio is unpriced.
- Click **Refresh Prices & Divs** once after updating so Account Change is recalculated.

### Double-Hedge Put Butterfly Scanner

- Added a 100-DTE 30/12/3-delta leg plan beside the CC4 25/15/2.5 plan, chosen with Leg plan buttons at the top of the General Option Scanner.
- A Ratio size picker scales the whole 1/-2/+2 ratio up to 20/-40/+40, with per-position dollar rules scaled to match. Starting point and Setup presets keep the plan, size, and debit or credit.
- An Opening debit / credit choice sets the upper expiration line. The sold body stays on its delta and the lower longs move to the highest strike that meets the limit. When the limit can't be reached, the plan's standard trade is shown with "Opening debit / credit" listed as a missed rule.
- A finish above the upper long that never tested the position now counts as a success.

## Also included from v1.36.2
...
**Changes since the last deployment**: https://github.com/pressj01/portfolio-Tracking-client/compare/v1.36.2...v1.36.3

**Full Changelog**: https://github.com/pressj01/portfolio-Tracking-client/compare/v1.33.7...v1.36.3
